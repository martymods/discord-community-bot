// ✅ tools/applyThemedNamesToUnclaimed.js
...
async function applyNames() {
  const unowned = await Property.find({ ownerId: null }).sort({ price: 1 }).limit(names.length);

  let updated = 0;
  for (let i = 0; i < unowned.length; i++) {
    unowned[i].name = names[i];
    await unowned[i].save();
    console.log(`✅ ${unowned[i].id} renamed to: ${names[i]}`);
    updated++;
  }

  console.log(`\n🎉 Done! ${updated} properties updated with themed names.`);
